...
async function getViewCountFromDynamoDB() {
  const params = {
...
jve7h7-codex/refactor-terminal-command-handling-for-interactivity
  ...
  try {
    ...
    if (command === 'view counter') {
      const count = await getViewCountFromDynamoDB();
      return {
        statusCode: 200,
        headers: HEADERS,
        body: `Total views: ${count}`
      };
    }
